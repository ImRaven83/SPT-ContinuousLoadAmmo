...
public class InventoryScreenClosePatch : ModulePatch
{
    ...
    /// </summary>
    [PatchPrefix]
    protected static void Prefix(ref InventoryController ___inventoryController)
    {
        if (!CommonUtils.InRaid) return;

        if (___inventoryController is Player.PlayerInventoryController playerInventoryController)
        {
