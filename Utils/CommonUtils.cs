...
public static class CommonUtils
{
    ...
    public static bool HasAnyHandsActionNonLinq(this ItemController traderController)
    {
        foreach (var eventArgs in traderController.ActiveEvents)
        {
            if (eventArgs is IItemInHandsEventArgs and not RemoveFromHandsEventArgs)
            {
                // (RemoveFromHandsEventArgs) RemoveFromHandsEventArgs - not considered as busy hands, for successive unloading of different bullets
                return true;
            }
        }
        return false;
